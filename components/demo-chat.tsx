...
export default function DemoChat(){
  ...
  async function send(e:FormEvent){e.preventDefault(); const value=text.trim(); if(!value||busy)return; setMessages(m=>[...m,{role:"user",content:value}]);setText("");setBusy(true);
    try{const res=await fetch("/api/demo/chat",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({message:value,history:messages})});const data=await res.json();setMessages(m=>[...m,{role:"bot",content:data.reply}]);}finally{setBusy(false)}
  }
